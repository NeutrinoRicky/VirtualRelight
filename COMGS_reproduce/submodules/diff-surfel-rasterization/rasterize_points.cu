	...
	const bool debug)
{
  ...
  if (S > MAX_FEATURES) {
	AT_ERROR("features dimension exceeds MAX_FEATURES");
  }
  ...
  torch::Tensor out_contrib = torch::full({1, H, W}, 0.0, int_opts);
	...
	const bool debug) 
{
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dfeatures, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations);
}
