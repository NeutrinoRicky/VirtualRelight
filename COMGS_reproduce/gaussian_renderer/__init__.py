...
def render_multitarget(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None, eps=1e-6):
    ...
    normal = torch.nn.functional.normalize(base["rend_normal"], dim=0, eps=eps)
    valid = (weight > eps).float()
    normal = torch.nan_to_num(normal, nan=0.0, posinf=0.0, neginf=0.0) * valid

    # Material maps are rendered via override colors so they share the same rasterization behavior.
    black_bg = torch.zeros_like(bg_color)
    albedo = render(
        viewpoint_camera=viewpoint_camera,
        pc=pc,
        pipe=pipe,
    ...
    rm_override = torch.cat([pc.get_roughness, pc.get_metallic, torch.zeros_like(pc.get_metallic)], dim=1)
    rm_render = render(
        viewpoint_camera=viewpoint_camera,
        pc=pc,
        pipe=pipe,
        bg_color=black_bg,
        scaling_modifier=scaling_modifier,
        override_color=rm_override,
    )["render"]

    roughness = rm_render[0:1]
    metallic = rm_render[1:2]
