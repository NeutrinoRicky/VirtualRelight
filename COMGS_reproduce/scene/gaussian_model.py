...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        if len(model_args) == 12:
            ...
            self._init_material_from_features()
        else:
            (
                self.active_sh_degree,
                self._xyz,
        ...
        try:
            ...
            print("[Warning] Optimizer state incompatible with current Gaussian parameters. Reinitializing optimizer state.")
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        fused_rgb = torch.tensor(np.asarray(pcd.colors)).float().cuda()
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, albedo, roughness, metallic, scale, rotation), axis=1)
    ...
    def load_ply(self, path):
        ...
        attribute_names = [p.name for p in plydata.elements[0].properties]
        albedo_names = sorted([n for n in attribute_names if n.startswith("albedo_")], key=lambda x: int(x.split('_')[-1]))
        if len(albedo_names) == 3:
        ...
        if "roughness" in attribute_names:
            roughness = np.asarray(plydata.elements[0]["roughness"])[..., np.newaxis]
        else:
            roughness = None

        if "metallic" in attribute_names:
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_albedo, new_roughness, new_metallic):
        d = {
            "xyz": new_xyz,
